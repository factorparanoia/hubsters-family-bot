...
function logEvent(guildId, payload) {
  appendJsonArray(`events-${guildId}`, {
    ...payload,
    timestamp: Date.now()
  });
}
...
client.on('messageReactionAdd', async (reaction, user) => {
  ...
  await member.roles.add(bind.roleId);
});
...
client.on('messageReactionRemove', async (reaction, user) => {
  ...
  const member = await reaction.message.guild.members.fetch(user.id);
  await member.roles.remove(bind.roleId);
});
...
client.on('interactionCreate', async (interaction) => {
  ...
  if (interaction.commandName === 'warnings') {
    const targetUser = interaction.options.getUser('target', true);
    const warnings = readJson(`warnings-${guildId}`, []).filter((row) => row.userId === targetUser.id);
  ...
  if (interaction.commandName === 'rank') {
    const rank = getRank(guildId, interaction.user.id);
  ...
  if (interaction.commandName === 'leveltop') {
    ...
    const top = getTop(guildId, limit);
    if (!top.length) {
    ...
    const text = top
      .map((row) => `${row.rank}. <@${row.userId}> — lvl ${row.level}, xp ${row.xp}`)
      .join('\n');
  ...
  if (interaction.commandName === 'config') {
    ...
    if (sub === 'welcome') {
      ...
      const cfg = setGuildConfig(guildId, patch);
      await interaction.reply(
    ...
    if (sub === 'logchannel') {
      ...
      const cfg = setGuildConfig(guildId, { logChannelId: channel.id });
    ...
    if (sub === 'autorole') {
      ...
      const cfg = setGuildConfig(guildId, { autoRoleId: role.id });
    ...
    if (sub === 'suggestions') {
      const channel = interaction.options.getChannel('channel', true);
      const cfg = setGuildConfig(guildId, { suggestionsChannelId: channel.id });
  ...
  if (interaction.commandName === 'reactionrole') {
    ...
    const role = interaction.options.getRole('role', true);
    upsertReactionRole(guildId, emoji, role.id);
  ...
  if (interaction.commandName === 'automod') {
    ...
    if (sub === 'links') {
      const enabled = interaction.options.getBoolean('enabled', true);
      setGuildConfig(guildId, { automod: { ...cfg.automod, linksBlocked: enabled } });
    ...
    if (sub === 'word_add') {
      ...
      setGuildConfig(guildId, { automod: { ...cfg.automod, blockedWords: words } });
  ...
  if (interaction.commandName === 'analytics') {
    const events = readJson(`events-${guildId}`, []);
